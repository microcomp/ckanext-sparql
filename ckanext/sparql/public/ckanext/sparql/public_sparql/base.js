...
function call_sparql_point_server() {
    ...
    $.ajax({
		...
		success: function(response) {
			...
			$('#field-link-sparql-server-query_json').val('http://' + window.location.hostname +port+ '/' + change_direct_link_value(this.url) + add_extra_fields_url('json'));
			$('#go_to_link_query_json').attr("href", 'http://' + window.location.hostname +port+ '/' + change_direct_link_value(this.url) + add_extra_fields_url('json'));
			
			$('#field-link-sparql-server-query_turtle').val('http://' + window.location.hostname +port+ '/' + change_direct_link_value(this.url) + add_extra_fields_url('turtle'));
			$('#go_to_link_query_turtle').attr("href", 'http://' + window.location.hostname +port+ '/' + change_direct_link_value(this.url) + add_extra_fields_url('turtle'));
			
			$('#field-link-sparql-server-query_csv').val('http://' + window.location.hostname +port+ '/' + change_direct_link_value(this.url) + add_extra_fields_url('csv'));
			$('#go_to_link_query_csv').attr("href", 'http://' + window.location.hostname +port+ '/' + change_direct_link_value(this.url) + add_extra_fields_url('csv'));
			
			$('#go_to_link_query_query').attr("href", 'http://' + window.location.hostname +port+ '/en/sparql?' + $.param({'view_code' : prefixes + editor.getValue()}));
